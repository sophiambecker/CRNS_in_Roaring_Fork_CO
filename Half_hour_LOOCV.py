...
# Extract row 16 and columns 1G«Ù15
...
publication_df = publication_df[['Fit', 'KGE', 'r', 'beta', 'gamma', 'r_squared', 'RMSE (g/g)', 'UbRMSE (g/g)', 'UbRMSE (cm^3/cm^3)']]
...
for n in site_names_new:
    ...
    if matches.empty:
        print(f"G‹·n+≈ No match for SampDate {samp_date} in daily data for {THIS_SITE_new}.")
        continue
    
    if len(matches) > 1:
        print(f"G‹·n+≈ Multiple matches for SampDate at site {THIS_SITE_new}. Using first one.")
