...
site_var = pd.read_csv(os.path.join(inDir, 'Calibration_AnalysisWithKGE_output_20251031\\Site_Calibration_data_summary.csv'))

#load parameter fits
Param = pd.read_csv(os.path.join(inDir, 'Calibration_AnalysisWithKGE_output_20251103\\Parameter_fit.csv'))
...
Q1_Des = np.percentile(site_var['Des_resid_singleN0'], 25)
Q3_Des = np.percentile(site_var['Des_resid_singleN0'], 75)
...
iqr_outliers_Des = [x for x in site_var['Des_resid_singleN0'] if x < lower_iqr_bound_Des or x > upper_iqr_bound_Des]
...
sd_resid_des = np.std(site_var['Des_resid_singleN0'])
mean_resid_des = np.mean(site_var['Des_resid_singleN0'])
upper_des = mean_resid_des + 2*sd_resid_des
lower_des = mean_resid_des - 2*sd_resid_des

# find outliers using inter quartile range
Q1_UTS = np.percentile(site_var['UTS_Resid_singleN0'], 25)
Q3_UTS = np.percentile(site_var['UTS_Resid_singleN0'], 75)
...
iqr_outliers_UTS = [x for x in site_var['UTS_Resid_singleN0'] if x < lower_iqr_bound_UTS or x > upper_iqr_bound_UTS]
print(f"Outliers: {iqr_outliers_UTS}")

# find outliers in the residuals # keep working on these: want to just find 1d outliers from residuals now

sd_resid_uts = np.std(site_var['UTS_Resid_singleN0'])
mean_resid_uts = np.mean(site_var['UTS_Resid_singleN0'])
upper_uts = mean_resid_uts + 2*sd_resid_uts
lower_uts = mean_resid_uts - 2*sd_resid_uts

plt.hist(site_var['UTS_Resid_singleN0'])
plt.show()

plt.hist(site_var['Des_resid_singleN0'])
plt.show()
# plot 1a
...
categories = site_var['Site'].astype("category").cat.categories
...
fig.legend(handles = [Line2D([0], [0], marker = 'none',  color='red', label="mean",
           ...
           labels = ['Mean', '2 St. Dev.', "IQR Bound",],
