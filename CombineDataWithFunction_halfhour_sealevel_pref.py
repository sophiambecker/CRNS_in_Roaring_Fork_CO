...
outDir = os.path.normpath(inDir + os.sep + 'CombineDataWithFunction_half_hr_output_sealevel_pref_'+stamp) + '\\'   # Set output directory
...
directory_path_nc = 'Data\\ModCountsProcessing_Des_output_sealevel_pref20260210'
...
directory_path_nc = 'Data\\ModCountsProcessing_UTS_output_sealevel_pref20260210'
file_pattern_nc = f'{directory_path_nc}/Mod*'
file_paths = glob.glob(file_pattern_nc, recursive=True)
dataframes_loc_UTS_f3 = [pd.read_csv(file_path) for file_path in file_paths]

# USGS data release
directory_path_usgs = 'C:\\Users\\sbecker14\\Documents\\GitHub\\CRNS_in_Roaring_Fork_CO\\Data\\Data_Release_2024_b'
...
def cleanlocal_df(start, end, df_loc):
    ...
    # compute daily averages
    # apply an adaptive filter to moderated counts based on per-file distribution
    counts = df_loc_filt['mod_nc_cph']
    if counts.notna().any():
        q01, q99 = counts.quantile([0.01, 0.99])
        pad = 500  # small cushion to avoid over-clipping
        lower = q01 - pad
        upper = q99 + pad
        # Set entire rows to NaN where mod_nc_cph is outside the adaptive range
    ...
    if 'Rhov_g_cm3' in df_loc_filt.columns:
        agg_col_loc = ['datetime_date', 'mod_nc_cph', 'swc', 'Rhov_g_cm3']
    else: agg_col_loc = ['datetime_date', 'mod_raw', 'mod_nc_cph', 'swc', 'Mod_cv', 'Mod_sqrt']
    
    #daily_avg_loc = df_loc_filt.groupby(df_loc_filt['datetime_date'].dt.date)[agg_col_loc].mean() #should ignore nan by default
    daily_avg_loc = df_loc_filt[agg_col_loc]
...
def combinedfs(oldname_str, newname_str, choose_start, choose_end):
    ...
    daily_avg_Des_f3.columns = ['datetime_date','Mod_raw_cph', 'Mod_cph_Des_f3', 'CRNS_SWC_Des_f3', 'Mod_cv', 'Mod_sqrt']
    daily_avg_UTS_f3 = cleanlocal_df(start = choose_start, end = choose_end, 
                                      df_loc = df_loc_UTS_f3)
    daily_avg_UTS_f3.columns = ['datetime_date','Mod_cph_UTS_f3', 'CRNS_SWC_UTS_f3', 'Rhov_g_cm3']
    ...
    agg_col_usgs = ['datetime_date', 'BaroPress', 'airRH', 'airT', 'N1_cts', 'N2_cts', 'SWC_5', 'SWC_10','SWC_20', 'SWC_50',
                    'sT_5', 'sT_10', 'sT_20', 'sT_50']
    
    #daily_avg_usgs = df_usgs_filt.groupby(df_usgs_filt['datetime_date'].dt.date)[agg_col_usgs].mean()
    daily_avg_usgs = df_usgs_filt[agg_col_usgs]
    ...
    print("Des_f3:", daily_avg_Des_f3.columns)
    print("UTS_f3:", daily_avg_UTS_f3.columns)
    print("USGS:", daily_avg_usgs.columns)
    
    daily_avg_Des_f3['datetime_date'].dt.tz
    daily_avg_UTS_f3['datetime_date'].dt.tz
    daily_avg_usgs['datetime_date'].dt.tz
    
    #remove any duplicate rows
    daily_avg_Des_f3 = daily_avg_Des_f3.groupby('datetime_date', as_index=False).mean()
    daily_avg_UTS_f3 = daily_avg_UTS_f3.groupby('datetime_date', as_index=False).mean()
    
    daily_comb1 = daily_avg_Des_f3.merge(daily_avg_UTS_f3, on='datetime_date', how='outer')
    print(f'Columns of daily_comb1 are {daily_comb1.columns}')
    daily_comb = daily_comb1.merge(daily_avg_usgs, on='datetime_date', how='outer')
    
    #rename columns
    daily_comb.columns = ['DateTime','Mod_raw_cph',
        'Mod_cph_Des_f3', 'CRNS_SWC_Des_f3', 'Mod_cv', 'Mod_sqrt',
        'Mod_cph_UTS_f3', 'CRNS_SWC_UTS_f3', 'Rhov_g_cm3'
    ...
    daily_comb_TDR = pd.concat([daily_comb, final_theta_series, theta_sd_series, 
                                theta_se_series], axis = 1, join='outer')
    ...
    keep = [ 'DateTime', 'Mod_raw_cph', 'Mod_cv', 'Mod_sqrt', 'Mod_cph_Des_f3', 'CRNS_SWC_Des_f3', 'Mod_cph_UTS_f3','CRNS_SWC_UTS_f3','Rhov_g_cm3',
            'BaroPress', 'SWC_5', 'SWC_10', 'SWC_20', 'SWC_50','WeightedTDR_SWC', 
            'WeightedTDR_SD', 'WeightedTDR_SE', 'sT_5', 'sT_10', 'sT_20', 'sT_50', 'BaroPress', 'airRH', 'airT',]
    
    daily_simple = daily_comb_TDR[keep]
    
    #rename colums
    
    new_col_names = ['DateTime', 'Raw_Moderated_cph', 'Raw_Mod_Coeff_of_Var', 'Raw_Mod_sqrt', 'Corrected_Mod_cph_for_Des', 'SWC_Des_cm3_cm3', 
    ...
    daily_simple.index.name = 'Date'
    daily_simple.to_csv(f'{outDir}{oldname_str}_{newname_str}_CRNS_Site_Data_half_hr.csv')
    print(f'{oldname_str} finished')
    
# loop through all of the sites with the function above
sitenames_df['Short_name'] = sitenames_df['Short_name'].astype(str).str.strip().str.replace('RF 5', 'RF5')
