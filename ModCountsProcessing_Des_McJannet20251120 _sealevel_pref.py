...
outFold = '\\ModCountsProcessing_Des_output_sealevel_pref{}'.format(stamp)
...
def process_local(new_site_name) -> None:
    ...
    df2['SiteName']= Site
    print(f"Success for site {new_site_name} on calibration day {calday}")
    return(df2, Site)
...
for s in site_names_new:
    ...
    if result is None:
       ...
       continue  # Skip the current iteration if result is NoneProcessed, name = process_lo
