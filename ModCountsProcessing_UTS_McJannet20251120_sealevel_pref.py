...
from helpers import get_correction_parameters, get_fi_over_time, datenum, clean_data_local, half_hour_string
...
Dir = "C:\\Users\\sbecker14\\Documents\\Github\\CRNS_in_Roaring_Fork_CO\\Data"
outFold = '\\ModCountsProcessing_UTS_output_sealevel_pref{}'.format(stamp)
...
def process_local(new_site_name) -> None:
    df = df_dict[new_site_name]
    # Extract the substring for the start date
    start_date = df['datetime_UTC'][0]#[0:10]
    ...
    df.rename(columns={"datetime_UTC": "message_date", "airT":"t7_c", "airRH":"h7_pct", 
               "N1_cts": "n1_persamp", "N2_cts": "n2_persamp", "N3_cts": "n3_persamp", "BaroPress":"p1_mb"}, inplace=True) 
    
    #df['message_date']= df['message_date'].apply(remove_utc_from_string_date)
    df['message_date']= df['message_date'].apply(lambda x: dt.datetime.strptime(x,'%Y-%m-%d %H:%M'))
    
    df['e1'] = 60*30 # seconds in measurement interval (30 minutes)
    ...
    if len(df_clean) == 0:
        print(f"No data found for site {new_site_name} on {start_date}")
        #return
    ...
    if np.isfinite(mod).mean() == 0 or np.isfinite(bare).mean() == 0:
        print(f"No valid data for site {new_site_name} on {start_date}")
        #return
    ...
    if not np.any(mask1):
        print(f"No valid data for site {new_site_name} on {start_date} (no times in mask)")
    ...
    df2["site"] = str(new_site_name)
    df2['SiteName']= Site
    
    print(f"Success for site {new_site_name} on calibration date {calday}")
    
    return(df2, Site, fscal, Lpt)


# directory path for raw data

directory_path = 'C:\\Users\\sbecker14\\Documents\\GitHub\\CRNS_in_Roaring_Fork_CO\\Data\\Data_Release_2024_b'


# Use glob with the ** wildcard to search recursively through subdirectories for .csv files
file_pattern = f'{directory_path}/*v1.csv'
...
df_dict = dict(zip(site_names_new, dataframes))

fscal_records = []
Lpt_records = []

for s in site_names_new:
    result = process_local(s)
    if result is None:
    ...
    Processed, name, fscal, lpt = result
    Processed.to_csv(f'{outDir}Mod_{name}.csv')
    fscal_records.append(
        {
            "site_code": s,
            "site_name": name,
            "fscal": fscal,
...
if fscal_records:
    fscal_df = pd.DataFrame(fscal_records)
    fscal_df.to_csv(os.path.join(outDir, "fscal_summary.csv"), index=False)
    
if Lpt_records:
    Lpt_df = pd.DataFrame(Lpt_records)
    Lpt_df.to_csv(os.path.join(outDir, "Lpt_summary.csv"), index=False)
