...
Dir = os.getcwd()
outFold = '\\N0_analyses_output_{}'.format(stamp)
...
directory_path_N0= 'Calibration_AnalysisWithCorrelation_output_20251205'
...
names = pd.read_excel('Data\\Data_Release_2024\\Network_paper_site_names.xlsx')
...
for s in site_names_new:
    ...
    N0_probe = cal_data['N0'].item()
    ND_probe = cal_data['ND'].item()
    ...
    SWC_probe_vol = cal_data['WeightedTDR_SWC'].item()
    
    Rhov_cal_g = cal_data['Rhov_g_cm3'].item()
